...
from carver_interfaces.action import NavigateThroughPoses
...
def get_quaternion_from_euler(ai, aj, ak):
        ai /= 2.0
        aj /= 2.0
        ak /= 2.0
        ci = math.cos(ai)
...
class MinimalService(Node):
    def __init__(self):
        ...
        self.srv1 = self.create_service(Goaly, '/initial', self.goal_callback1)
        self.srv2 = self.create_service(Goaly, '/goal', self.goal_callback2)
    ...
    def goal_callback1(self, request1, response1):
        response1.result = True
        self.get_logger().info("initial_pose")
    ...
    def goal_callback2(self, request2, response2):
        ...
        goal_pose.pose.position.x = request2.x
        goal_pose.pose.position.y = request2.y
        goal_pose.pose.position.z = request2.z
        q = get_quaternion_from_euler(request2.roll, request2.pitch, request2.yaw)
